...
defineFeature(feature, (test) => {
  test("When user has not specified a number, 32 is the default number", ({
    ...
    given(
      "the user did not specify the number of events being shown",
      () => {}
    );
  ...
  test("User can change the number of events they want to see", ({
    ...
    let AppWrapper;
    given(
      "the list of elements have been loaded and the user did not specify a number of events he wants to see",
      () => {
        AppWrapper = mount(<App />);
      }
    );
